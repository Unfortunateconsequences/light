...
SSID, password, controller IP and UDP port are **not** C++ literals in `config.hpp`. They come from ESP-IDF Kconfig. `idf.py build` / `menuconfig` writes **`esp32/sdkconfig`** (no `.h`). IDF injects `CONFIG_*` into every compile unit; C++ must not `#include "sdkconfig.h"` and must not `#define CONFIG_*`.

```
esp32/main/Kconfig.projbuild   # menu "Light Sensor Configuration"
        ↓
esp32/sdkconfig.defaults       # short overlay in git (empty SSID/PASS)
        ↓
esp32/sdkconfig                # generated, gitignored — secrets live only here
        ↓
CONFIG_LIGHT_SENSOR_*          # visible in C++ automatically
        ↓
include/config.hpp             # aliases WIFI_SSID / WIFI_PASS / CONTROLLER_*
```

`sdkconfig.defaults` stays a **short overlay** (4 MB flash, 240 MHz, empty Wi-Fi). Do **not** commit a dumped `sdkconfig`: it is machine-local, can pin 2 MB / 160 MHz, and may contain the real password.
